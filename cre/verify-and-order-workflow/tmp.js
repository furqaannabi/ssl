...
var sha256 = /* @__PURE__ */ createHasher2(() => new _SHA256, /* @__PURE__ */ oidNist(1));
...
function numberToBytesBE(n, len2) {
  anumber2(len2);
  n = abignumber(n);
  const res = hexToBytes3(n.toString(16).padStart(len2 * 2, "0"));
  if (res.length !== len2)
...
function validateObject(object, fields2 = {}, optFields = {}) {
  ...
  const iter = (f, isOpt) => Object.entries(f).forEach(([k, v]) => checkField2(k, v, isOpt));
  iter(fields2, false);
  iter(optFields, true);
}
...
var buyOrders = [];
var sellOrders = [];
function generateStealthAddress(spendingPublicKeyHex, ephemeralSeed) {
  const ephemeralPrivBytes = toBytes(keccak256(toBytes(ephemeralSeed)));
  const ephemeralPublicKey = secp256k1.getPublicKey(ephemeralPrivBytes, false);
  ...
  const stealthPubBytes = stealthPoint.toBytes(false);
...
function hexToUint8Array(hex) {
  ...
  const bytes = new Uint8Array(clean3.length / 2);
  for (let i2 = 0;i2 < bytes.length; i2++) {
    bytes[i2] = parseInt(clean3.substring(i2 * 2, i2 * 2 + 2), 16);
  }
  return bytes;
}
...
function matchOrders() {
  for (let i2 = 0;i2 < buyOrders.length; i2++) {
    for (let j = 0;j < sellOrders.length; j++) {
      const buy = buyOrders[i2];
      const sell = sellOrders[j];
      if (buy.order.asset === sell.order.asset && BigInt(buy.order.price) >= BigInt(sell.order.price)) {
        buyOrders.splice(i2, 1);
...
var onHttpTrigger = (runtime2, payload) => {
  ...
  if (data.action === "verify") {
    runtime2.log("Verify request for nullifier: " + data.nullifierHash);
    const reportData2 = encodeAbiParameters(parseAbiParameters("uint8 reportType, uint256 nullifierHash"), [0, BigInt(data.nullifierHash)]);
    const txHash2 = sendReport2(runtime2, reportData2);
    runtime2.log("Verify tx: " + txHash2);
  ...
  runtime2.log("Order received: " + data.side + " " + data.amount + " @ " + data.price);
  const order = {
    nullifierHash: data.nullifierHash,
    order: {
      asset: data.asset,
  ...
  storeOrder(order);
  runtime2.log("Order stored. Buy pool: " + buyOrders.length + " Sell pool: " + sellOrders.length);
  const match = matchOrders();
  if (!match) {
    runtime2.log("No match — queued");
    return JSON.stringify({ status: "queued", side: data.side });
  }
  runtime2.log("Match found!");
  ...
  const tradeNonce = keccak256(encodePacked(["string", "string", "string"], [match.buyer.nullifierHash, match.seller.nullifierHash, Date.now().toString()]));
  const buyerStealth = generateStealthAddress(match.buyer.stealthPublicKey, tradeNonce + "_buyer");
  const sellerStealth = generateStealthAddress(match.seller.stealthPublicKey, tradeNonce + "_seller");
  runtime2.log("Stealth buyer: " + buyerStealth.stealthAddress);
  runtime2.log("Stealth seller: " + sellerStealth.stealthAddress);
  const orderId = keccak256(encodePacked(["string", "string", "string"], [tradeNonce, buyerStealth.stealthAddress, sellerStealth.stealthAddress]));
  const reportData = encodeAbiParameters(parseAbiParameters("uint8 reportType, bytes32 orderId, address stealthBuyer, address stealthSeller, uint256 buyerNullifier, uint256 sellerNullifier, address tokenA, address tokenB, uint256 amountA, uint256 amountB"), [
    1,
    orderId,
    buyerStealth.stealthAddress,
    sellerStealth.stealthAddress,
    BigInt(match.buyer.nullifierHash),
    BigInt(match.seller.nullifierHash),
    match.seller.order.asset,
    match.seller.order.quoteToken,
    tradeAmount,
    tradeAmount
  ]);
