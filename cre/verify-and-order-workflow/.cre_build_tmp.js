...
init_toBytes();
init_keccak256();
function getPrimaryChain(config) {
  return config.chains[config.primaryChain];
}
function findChainBySelector(config, selectorName) {
  return Object.values(config.chains).find((c) => c.chainSelector === selectorName);
}
function hashToField(input) {
  const hash2 = BigInt(keccak256(toBytes(input))) >> 8n;
  return `0x${hash2.toString(16).padStart(64, "0")}`;
}
function sendReportToChain(runtime2, reportData, chainSelectorName, vaultAddress) {
...
function sendReport2(runtime2, reportData) {
  const primary = getPrimaryChain(runtime2.config);
  return sendReportToChain(runtime2, reportData, primary.chainSelector, primary.vault);
}
var IS_VERIFIED_ABI = [{
  name: "isVerified",
  type: "function",
...
function checkIsVerified(runtime2, chainCfg, userAddress) {
  const network248 = getNetwork({
    chainFamily: "evm",
  ...
  if (!network248) {
    runtime2.log("Network not found for isVerified check on " + chainCfg.chainSelector + ", assuming not verified");
    return false;
  }
  const evmClient = new ClientCapability(network248.chainSelector.selector);
  ...
  const registryTarget = chainCfg.worldIdRegistry || chainCfg.vault;
  ...
  const hexData = "0x" + Buffer.from(reply.data).toString("hex");
  const [verified] = decodeAbiParameters([{ type: "bool" }], hexData);
  return verified;
}
var verifyProof = (sendRequester, config, data) => {
  const bodyData = {
...
var onHttpTrigger = (runtime2, payload) => {
  ...
  if (data.action === "verify") {
    runtime2.log("Verify request for nullifier: " + data.nullifier_hash + ", address: " + data.userAddress);
    const httpClient = new ClientCapability2;
    const verificationResult = httpClient.sendRequest(runtime2, (sender, cfg) => verifyProof(sender, cfg, data), consensusIdenticalAggregation())(runtime2.config).result();
    if (verificationResult.statusCode < 200 || verificationResult.statusCode >= 300) {
      ...
      const alreadyUsed = verificationBody.includes("max_verifications_reached") || verificationBody.includes("already_verified") || verificationBody.includes("exceeded_max_verifications");
      if (!alreadyUsed) {
        runtime2.log("Verification failed: status=" + verificationResult.statusCode + " body=" + compactBody);
      ...
      runtime2.log("World ID reports already verified/max verifications — proceeding with on-chain report");
    }
    const reportData = encodeAbiParameters(parseAbiParameters("uint8 reportType, address user"), [0, data.userAddress]);
    const allChains = Object.entries(runtime2.config.chains);
    const targetChains = data.selectedChains && data.selectedChains.length > 0 ? allChains.filter(([chainName]) => data.selectedChains.includes(chainName)) : allChains;
    if (targetChains.length === 0) {
      runtime2.log("No matching chains found for selectedChains: " + JSON.stringify(data.selectedChains));
    }
    const chainResults = {};
    for (const [chainName, chainCfg] of targetChains) {
      if (!chainCfg.vault) {
        runtime2.log(`No vault deployed on ${chainName}, skipping`);
        chainResults[chainName] = "no_vault";
        continue;
      }
      let alreadyVerified = false;
      try {
        alreadyVerified = checkIsVerified(runtime2, chainCfg, data.userAddress);
      } catch (err) {
        runtime2.log(`isVerified check FAILED on ${chainName}: ${err.message} — proceeding with report`);
        alreadyVerified = false;
      }
      if (alreadyVerified) {
        runtime2.log(`User already verified on ${chainName}, skipping`);
        chainResults[chainName] = "already_verified";
        continue;
      }
      const reportTarget = chainCfg.worldIdRegistry || chainCfg.vault;
      try {
        const txHash = sendReportToChain(runtime2, reportData, chainCfg.chainSelector, reportTarget);
        chainResults[chainName] = txHash;
        runtime2.log(`Verify tx on ${chainName} → ${reportTarget}: ${txHash}`);
      } catch (err) {
        runtime2.log(`Verify tx FAILED on ${chainName}: ${err.message}`);
  ...
  if (data.action === "settle_match") {
    runtime2.log("Settlement request for " + data.buyer.orderId + " <-> " + data.seller.orderId);
    const tradeAmount = BigInt(data.tradeAmount);
    const quoteAmount = BigInt(data.quoteAmount);
    const buyerStealthAddr = data.buyer.stealthAddress;
    const sellerStealthAddr = data.seller.stealthAddress;
    runtime2.log("Stealth buyer: " + buyerStealthAddr);
    runtime2.log("Stealth seller: " + sellerStealthAddr);
    const orderId = keccak256(encodePacked(["string", "string", "string", "string"], [data.buyer.orderId, data.seller.orderId, buyerStealthAddr, sellerStealthAddr]));
    if (data.crossChain && data.destChainSelector) {
      runtime2.log("Cross-chain settlement detected");
      const primary = getPrimaryChain(runtime2.config);
      const sourceChain = data.sourceChainSelector || primary.chainSelector;
      const destChain = data.destChainSelector;
      const sourceCfg = findChainBySelector(runtime2.config, sourceChain);
      const destCfg = findChainBySelector(runtime2.config, destChain);
      if (!sourceCfg || !sourceCfg.vault)
        throw new Error("Source chain not configured: " + sourceChain);
      if (!destCfg || !destCfg.ccipReceiver)
        throw new Error("Dest chain ccipReceiver not configured: " + destChain);
      const ccipDestSelectorStr = data.ccipDestSelector || destCfg.ccipChainSelector;
      if (!ccipDestSelectorStr)
        throw new Error("Cannot determine CCIP dest selector for " + destChain);
      runtime2.log("CCIP dest selector: " + ccipDestSelectorStr + " (source: " + (data.ccipDestSelector ? "payload" : "config") + ")");
      ...
      const bridgeReport = encodeAbiParameters(parseAbiParameters("uint8 reportType, bytes32 orderId, uint64 destChainSelector, address destReceiver, address buyer, address seller, address usdcToken, uint256 usdcAmount, address rwaToken, uint256 rwaAmount"), [
        3,
        orderId,
      ...
      runtime2.log("Sending crossChainSettle (type=3) to " + sourceChain + " vault " + sourceVault);
      const bridgeTxHash = sendReportToChain(runtime2, bridgeReport, sourceChain, sourceVault);
      runtime2.log("Bridge tx: " + bridgeTxHash);
      return JSON.stringify({
    ...
    const reportData = encodeAbiParameters(parseAbiParameters("uint8 reportType, bytes32 orderId, address stealthBuyer, address stealthSeller, address tokenA, address tokenB, uint256 amountA, uint256 amountB"), [
      1,
      orderId,
      buyerStealthAddr,
      sellerStealthAddr,
    ...
    const baseCfg = findChainBySelector(runtime2.config, data.baseChainSelector);
    if (!baseCfg)
      throw new Error("Base chain not configured: " + data.baseChainSelector);
    if (!baseCfg.vault)
      throw new Error("No vault deployed on chain: " + data.baseChainSelector);
    const txHash = sendReportToChain(runtime2, reportData, baseCfg.chainSelector, baseCfg.vault);
    runtime2.log("Settlement tx: " + txHash);
    return JSON.stringify({
      status: "settled",
      orderId,
      stealthBuyer: buyerStealthAddr,
  ...
  if (data.action === "withdraw") {
    runtime2.log("Withdrawal request " + data.withdrawalId + " for " + data.userAddress);
    const reportData = encodeAbiParameters(parseAbiParameters("uint8 reportType, address user, uint256 withdrawalId"), [2, data.userAddress, BigInt(data.withdrawalId)]);
    const txHash = sendReport2(runtime2, reportData);
    runtime2.log("Withdrawal tx: " + txHash);
    return JSON.stringify({
      status: "withdrawn",
