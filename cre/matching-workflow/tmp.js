  ...
  class Signature {
    ...
    recoverPublicKey(messageHash) {
      ...
      if (!Fp.isValid(radj))
        throw new Error("recovery id 2 or 3 invalid");
      const x = Fp.toBytes(radj);
      const R = Point.fromBytes(concatBytes(pprefix2((rec & 1) === 0), x));
      const ir = Fn.inv(radj);
...
var secp256k12 = createCurve2({ ...secp256k1_CURVE2, Fp: Fpk12, lowS: true, endo: secp256k1_ENDO2 }, sha256);
init_secp256k1();
init_toHex();
init_address();
init_isAddress();
...
function setBigUint642(view, byteOffset, value2, isLE3) {
  if (typeof view.setBigUint64 === "function")
...
function computeTag(fn, isLE3, key, data, AAD) {
  ...
  if (AAD)
    setBigUint642(view, 0, BigInt(aadLength * 8), isLE3);
  setBigUint642(view, 8, BigInt(data.length * 8), isLE3);
  h.update(num);
  const res = h.digest();
  clean2(num);
  return res;
...
var limit = (name, min, max) => (value2) => {
  if (!Number.isSafeInteger(value2) || min > value2 || value2 > max)
    throw new Error(`${name}: invalid value=${value2}, must be [${min}..${max}]`);
};
    ...
    if (out.length === 16)
      ...
      for (let j = 0, ctr3 = 1;j < 6; j++) {
        for (let pos = 2;pos < o32.length; pos += 2, ctr3++) {
          ...
          a0 = s0, a1 = s1 ^ byteSwap2(ctr3), o32[pos] = s2, o32[pos + 1] = s3;
        }
      }
      o32[0] = a0, o32[1] = a1;
    }
...
function checkIsVerifiedOnChain(runtime2, userAddress) {
  const registryAddress = runtime2.config.worldIdRegistry;
  if (!registryAddress) {
    runtime2.log("[TEE] worldIdRegistry not configured — skipping isVerified check");
    return true;
  }
  const chainSelector = runtime2.config.ethSepoliaChainSelector || "ethereum-testnet-sepolia";
  ...
  if (!network248) {
    runtime2.log("[TEE] Network not found for isVerified check — skipping");
    return true;
  }
  ...
  const reply = evmClient.callContract(runtime2, {
    call: {
      to: hexToBase64(registryAddress),
...
function getServiceAddress(runtime2) {
  const key = runtime2.config.servicePrivateKey.startsWith("0x") ? runtime2.config.servicePrivateKey : `0x${runtime2.config.servicePrivateKey}`;
  return privateKeyToAccount(key).address;
}
function signTransfer(runtime2, params) {
  ...
  const privKeyHex = runtime2.config.servicePrivateKey.startsWith("0x") ? runtime2.config.servicePrivateKey.slice(2) : runtime2.config.servicePrivateKey;
  const sig = secp256k12.sign(hexToBytes4(hash2), privKeyHex);
  const r = sig.r.toString(16).padStart(64, "0");
  const s = sig.s.toString(16).padStart(64, "0");
  const v = (sig.recovery + 27).toString(16).padStart(2, "0");
  return `0x${r}${s}${v}`;
}
function toWeiStr(decimalAmt, decimals) {
  const str = decimalAmt.toFixed(decimals);
  const [int, frac = ""] = str.split(".");
  return int + frac.padEnd(decimals, "0");
}
function decryptOrder(encryptedBase64, privateKeyHex) {
  ...
  const sharedPoint = secp256k12.getSharedSecret(hexToBytes4(privateKeyHex), ephemeralPub, true);
  const sharedX = sharedPoint.slice(1);
  const aesKey = sha2562(sharedX);
...
function findMatch(newOrder, resting) {
  if (newOrder.side === "BUY") {
    ...
    const candidates = resting.filter((o) => o.side === "BUY" && parseFloat(o.price) >= parseFloat(newOrder.price)).sort((a, b) => parseFloat(b.price) - parseFloat(a.price));
    return candidates[0] ?? null;
  }
}
function httpCall(sender, url, method, body, headers) {
  ...
  const bodyOut = resp.body && resp.body.length > 0 ? new TextDecoder().decode(resp.body) : "";
...
var onHttpTrigger = (runtime2, payload) => {
  ...
  runtime2.log("[TEE] Received match_order orderId=" + input.orderId);
  let newOrder;
  try {
    newOrder = decryptOrder(input.encryptedOrder, runtime2.config.creDecryptionKey);
    newOrder.id = input.orderId;
  } catch (err) {
    runtime2.log("[TEE] Decryption failed: " + err);
  ...
  try {
    ...
    for (const rec of parsed.orders ?? []) {
      if (rec.id === input.orderId || !rec.encryptedPayload)
        continue;
      try {
        const d = decryptOrder(rec.encryptedPayload, runtime2.config.creDecryptionKey);
        d.id = rec.id;
        restingOrders.push(d);
      } catch {}
    }
  } catch (err) {
    runtime2.log("[TEE] Order book parse error: " + err);
  }
  runtime2.log("[TEE] Resting orders: " + restingOrders.length);
  const matchedOrder = findMatch(newOrder, restingOrders);
  if (!matchedOrder) {
    runtime2.log("[TEE] No match — order pending");
  ...
  if (!checkIsVerifiedOnChain(runtime2, buyerAddr)) {
    runtime2.log("[TEE] Buyer " + buyerAddr + " not World ID verified — aborting match");
    return JSON.stringify({ status: "pending", orderId: input.orderId, reason: "buyer_not_verified" });
  }
  if (!checkIsVerifiedOnChain(runtime2, sellerAddr)) {
    runtime2.log("[TEE] Seller " + sellerAddr + " not World ID verified — aborting match");
  ...
  const isValidAddress = (a) => typeof a === "string" && /^0x[0-9a-fA-F]{40}$/.test(a);
  const settleInfoRes = httpClient.sendRequest(runtime2, (sender, cfg) => httpCall(sender, cfg.backendUrl + "/api/order/settle-info?buyerOrderId=" + buyer.id + "&sellerOrderId=" + seller.id + "&pairId=" + input.pairId, "GET", undefined, { "X-CRE-Secret": cfg.callbackSecret }), consensusIdenticalAggregation())(runtime2.config).result();
  ...
  let quoteDecimals = 6;
  try {
    const info = JSON.parse(settleInfoRes.body);
    ...
    runtime2.log("[TEE] Failed to parse settle-info: " + e);
    return JSON.stringify({ status: "pending", orderId: input.orderId, reason: "settle_info_fetch_failed" });
  }
  runtime2.log("[TEE] Shields — buyer=" + buyerShieldAddress + " seller=" + sellerShieldAddress);
  if (!isValidAddress(buyerShieldAddress) || !isValidAddress(sellerShieldAddress)) {
    runtime2.log("[TEE] Missing shield address — skipping match");
    return JSON.stringify({ status: "pending", orderId: input.orderId, reason: "missing_shield_address" });
  }
  if (!baseTokenAddress || !quoteTokenAddress) {
    runtime2.log("[TEE] Token addresses missing");
    return JSON.stringify({ status: "pending", orderId: input.orderId, reason: "token_meta_missing" });
  }
  const serviceAccount = getServiceAddress(runtime2);
  const timestamp = Math.floor(Date.now() / 1000);
  const baseAmountWei = toWeiStr(tradeAmt, baseDecimals);
  const quoteAmountWei = toWeiStr(quoteAmt, quoteDecimals);
  runtime2.log("[TEE] Settling: " + baseAmountWei + " base -> buyer, " + quoteAmountWei + " quote -> seller");
  const buyerAuth = signTransfer(runtime2, {
    sender: serviceAccount,
    recipient: buyerShieldAddress,
    token: baseTokenAddress,
    amount: baseAmountWei,
    flags: ["hide-sender"],
    timestamp
  });
  const buyerTxRes = httpClient.sendRequest(runtime2, (sender, cfg) => httpCall(sender, cfg.convergenceApiUrl + "/private-transfer", "POST", {
  ...
  runtime2.log("[TEE] BuyerTx HTTP " + buyerTxRes.statusCode + " — " + buyerTxRes.body);
  let buyerTxId = null;
  try {
    buyerTxId = JSON.parse(buyerTxRes.body).transaction_id ?? null;
  } catch {}
  const sellerAuth = signTransfer(runtime2, {
    sender: serviceAccount,
    recipient: sellerShieldAddress,
    token: quoteTokenAddress,
    amount: quoteAmountWei,
    flags: ["hide-sender"],
    timestamp: timestamp + 1
  });
  const sellerTxRes = httpClient.sendRequest(runtime2, (sender, cfg) => httpCall(sender, cfg.convergenceApiUrl + "/private-transfer", "POST", {
    account: serviceAccount,
    recipient: sellerShieldAddress,
    token: quoteTokenAddress,
    amount: quoteAmountWei,
  ...
  runtime2.log("[TEE] SellerTx HTTP " + sellerTxRes.statusCode + " — " + sellerTxRes.body);
  let sellerTxId = null;
  try {
